...
object DeltaExampleOBS {
  ...
  private var solution = "delta"

  private val targetTablePath = "obs://obs-david/warehouse/target_table_delta"
  private val changeTablePath = "obs://obs-david/warehouse/change_table_delta"
  ...
  private def generateTarget(sparkSession: SparkSession): Unit = {
    print("================= generating delta target table...")
    val time = timeIt { () =>
      ...
      println("start to write init delta_target table")
      targetData.repartition(8)
        .write
        .format("delta")
        .mode(SaveMode.Overwrite)
        .save(targetTablePath)
      targetData.createOrReplaceTempView("delta_target")
    }
    println(s"================= generated delta_target table ${timeFormatted(time)}")
  }
  ...
  private def generateChange(sparkSession: SparkSession): Unit = {
    ...
    df.createOrReplaceTempView("delta_change")
  }
  ...
  private def printTarget(spark: SparkSession, i: Int) = {
    if (printDetail) {
      ...
      spark.sql("select count(*) from delta_target").show(false)
      val target = DeltaTable.forPath(targetTablePath)
      println("+++++++++++++++++++++++" + target.toDF.count())
    }
  }
  ...
  private def printChange(spark: SparkSession, i: Int) = {
    if (printDetail) {
      ...
      spark.sql("select count(*) from delta_change").show()
    }
  }

  private def createSession(carbonProPath: String) = {
    import org.apache.spark.sql.CarbonSession._
    val spark = CarbonSessionUtil.createCarbonSession("DeltaExampleOBS", carbonProPath)
    spark
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (printDetail) {
      println("## -- delta_target table")
      println("## ============ init delta_target table")
      spark.sql("select count(*) from delta_target").show()
    }

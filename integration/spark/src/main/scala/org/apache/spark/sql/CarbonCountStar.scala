...
case class CarbonCountStar(
  ...
  override def doExecute(): RDD[InternalRow] = {
    ...
    if (predicates.nonEmpty) {
      totalRowCount = getRowCountPurePartitionPrune
    } else {
      // get row count
          ...
          override def call(): Long = {
            ...
            prunedIndexFilesCurrentSeg.asScala.foreach(prunedIndexFilePath =>
              if (tableSegmentIndexes.get(segmentPath).getPrunedIndexFileToRowCountMap
                .get(prunedIndexFilePath) != null) {
                rowCountCurrentSeg += tableSegmentIndexes.get(segmentPath)
                  .getPrunedIndexFileToRowCountMap.get(prunedIndexFilePath)
              } else {

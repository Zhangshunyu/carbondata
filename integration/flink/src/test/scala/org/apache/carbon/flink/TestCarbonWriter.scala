...
class TestCarbonWriter extends QueryTest {
  ...
  test("test carbon writer of bucket table") {
    sql(s"DROP TABLE IF EXISTS $tableName").collect()
    sql(s"DROP TABLE IF EXISTS $tableName2").collect()
    sql(
      s"""
         | CREATE TABLE $tableName (stringField string, intField int, shortField short)
         | STORED AS carbondata TBLPROPERTIES ('BUCKETNUMBER'='10', 'BUCKETCOLUMNS'='stringField')
      """.stripMargin
    ).collect()
    sql(
      s"""
         | CREATE TABLE $tableName2 (stringField string, intField int, shortField short)
         | STORED AS carbondata TBLPROPERTIES ('BUCKETNUMBER'='10', 'BUCKETCOLUMNS'='stringField')
      """.stripMargin
    ).collect()
    ...
    try {
      ...
      sql(s"insert into $tableName2 select * from $tableName").collect()
